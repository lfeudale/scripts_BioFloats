...
#########################################
	    ...
	    name_fig = ''.join(['test_float_',wmolist,'_',str(season_text),'.png'])
	    name_fig_DCM = ''.join(['DCM_FLOAT_',wmolist,'_',str(season_text),'.png'])
	    fig_title_DCM= ''.join(['DCM ',str(len(Float)), ' FLOAT: ',wmolist ,' ',str(season_text2)])

	    fig_name_INTEGRAL = ''.join(['INTEGRAL_',wmolist,'_',str(season_text),'.png' ])
	    fig_title_INTEGRAL = ''.join(['INTEGRAL 0-200m for FLOAT ',wmolist ,':',str(season_text2) ]) 
                
            fig_title= ''.join(['CHLA ',str(len(Float)), ' FLOAT: ',wmolist ,' ',str(season_text2)])
...
#OK		   pl.scatter(Prof[ii200],Pres[ii200])
...
#OK                   pl.plot(NewProf_1m_RAve,NewPres_1m)
#OK                   pl.axis([0, 2.1, 0, 200])
#OK		   pl.title(fig_title)
...
#	    ax1.plot(Float_cycle,DCM,'G')
		...
		if (np.invert(np.isnan(sum(DCM)))) :
	    		ax1.plot(TimARR,DCM,'G')
	    		ax1.set_ylabel(r"DCM mg/m3",fontsize=16,color="green")
	    		ax2=ax1.twinx()
#	    ax2.plot(Float_cycle,Max_Depth,'b')
	    		ax2.plot(TimARR,Max_Depth,'b')
	    		ax2.set_ylabel(r"Depth of DCM m",fontsize=16,color="blue")
 	    		pl.title(fig_title_DCM)
#	    if check_na == 0 :
