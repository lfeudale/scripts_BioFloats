...
# CREATE THE LIST OF PROFILES FOR A SINGLE FLOAT
	  ...
	  if wmolist == pp._my_float.wmo :
	   ...
	   All_Profiles = np.zeros((len(Float),41,len(varname)), np.float64)
	   integral_CumulativeSum =  np.zeros((40,), np.float64)
	   MEAN1 = np.zeros((41,len(varname), np.float64)
           STD1 = np.zeros((41,len(varname)), np.float64)
	   DCM1 = np.zeros((41,len(varname)), np.float64)
           MAX1 = np.zeros((41,len(varname)), np.float64)
